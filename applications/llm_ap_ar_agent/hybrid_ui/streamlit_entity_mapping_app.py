...
load_dotenv()
from agents.map_extracted_text_to_structured_data_with_confidence_score import map_extracted_text_to_invoice_data_with_confidence_score
from agents.map_extracted_text_to_structured_data_with_confidence_score import map_extracted_text_to_sow_data_with_confidence_score
...
if extracted_text:
    with st.spinner("Classifying..."):
        ...
        if result == "sow":
            ...
            raw_vendor = mapped.get("vendor", "")
            vendor = str(raw_vendor).strip() if raw_vendor else ""
            ...
            prior_guidance = get_last_sow_correction(contracting_company, vendor, "SOW") if contracting_company else ""
            ...
            if submit:
                ...
                if contracting_company and combined_guidance.strip():
                    insert_sow_correction(contracting_company, vendor, "SOW", combined_guidance.strip())
            ...
            if "invoice_mapping" not in st.session_state:
                st.session_state.invoice_mapping = map_extracted_text_to_invoice_data_with_confidence_score(extracted_text)
            mapped = st.session_state.invoice_mapping
            raw_company = mapped.get("vendor", "")
            vendor = str(raw_company).strip() if raw_company else ""
            form = st.form("mapping_form")
            field_updates = {}
            for key, val in st.session_state.invoice_mapping.items():
                value = val.get("value") if isinstance(val, dict) else val
                field_updates[key] = form.text_input(f"{key.replace('_', ' ').title()}", value=value)

            if not vendor:
                st.warning("⚠️ vendor could not be identified. Corrections will not be saved.")
            else:
                st.info(f"📌 vendor: `{vendor}`")

            # 📥 Pre-fill with previous correction (if any)
            prior_guidance = get_last_sow_correction("netowen", vendor, "invoice") if vendor else ""
            st.info(f"📌 Prior Guidance: `{prior_guidance}`")
            additional_prompt_text = form.text_area("🧠 Additional Prompt Guidance (optional)", value=prior_guidance)
            submit = form.form_submit_button("🔁 Re-map with My Corrections")
            ...
            if submit:
                ...
                st.session_state.sow_mapping = map_extracted_text_to_sow_data_with_confidence_score(extracted_text, combined_guidance)
                st.success("🔁 Re-mapped using your corrections and guidance.")
                if vendor and combined_guidance.strip():
                    insert_sow_correction("netowen", vendor, "invoice", combined_guidance.strip())
                st.success("✅ Correction saved to DB.")
            ...
            raise ValueError(f"❌ Unsupported document type: {result}")