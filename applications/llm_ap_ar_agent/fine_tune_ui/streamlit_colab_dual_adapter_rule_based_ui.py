# app.py — Mistral 7B + dual LoRA adapters → JSON mapper (Invoice/SOW)
# GPU + bitsandbytes 4-bit, Toolchain integration, batch export, VRAM cleanup
# Adapters:
#  - "extract"     : for PDF ➜ JSON mapping
#  - "toolchain"   : for optional LLM-driven toolchain steps (if you choose to use LLM in tools)
...
from rule_based_tool_chain import (
...
# Page / defaults
# ----------------------------
st.set_page_config(page_title="Mistral JSON Mapper (GPU 4-bit) + Toolchain (Dual Adapters)", page_icon="⚡", layout="wide")
...
# ----------------------------
# Dual-adapter loader (GPU + bnb 4-bit) — meta-safe
# ----------------------------
@st.cache_resource(show_spinner=True)
def load_models_gpu_dual(base_model: str, adapter_extract: Optional[str], adapter_tool: Optional[str]):
    ...
    # Instantiate directly on device map to avoid meta-copy issues
    ...
    # Attach first adapter (extraction) as the main PEFT wrapper
    peft_model = base
    active_adapter_name = None

    if adapter_extract and os.path.isdir(adapter_extract):
        peft_model = PeftModel.from_pretrained(
            base,
            adapter_extract,
            adapter_name="extract",
            device_map="auto",
            torch_dtype=compute_dtype,
            is_trainable=False,
        ).eval()
        active_adapter_name = "extract"

    # Load second adapter (toolchain) into the same model
    if adapter_tool and os.path.isdir(adapter_tool):
        peft_model.load_adapter(
            adapter_tool,
            adapter_name="toolchain",
            is_trainable=False,
        )
        if active_adapter_name is None:
            active_adapter_name = "toolchain"

    # Set whichever adapter should be active by default
    if active_adapter_name:
        peft_model.set_adapter(active_adapter_name)

    return tok, peft_model


def load_models_gpu_dual_metasafe(base_model: str, adapter_extract: str|None, adapter_tool: str|None):
    assert torch.cuda.is_available(), "CUDA/GPU not available."

    # prefer bf16; fall back to fp16
    compute_dtype = torch.bfloat16
    try:
        _ = torch.zeros(1, dtype=compute_dtype, device="cuda")
    except Exception:
    ...
    # IMPORTANT: instantiate directly on GPUs; no later .to()
    base = AutoModelForCausalLM.from_pretrained(
        base_model,
        device_map="auto",
        torch_dtype=compute_dtype,
        ...
        attn_implementation="eager", # only needed if you plan to inspect attentions
    ).eval()

    # Attach first adapter (extraction) — wrap as PEFT
    peft_model = base
    if adapter_extract and os.path.isdir(adapter_extract):
        peft_model = PeftModel.from_pretrained(
            base,
            adapter_extract,
            adapter_name="extract",
            device_map="auto",        # <- ensure materialized on device
            torch_dtype=compute_dtype,
            is_trainable=False,
        ).eval()

    # Load second adapter (toolchain) into same model wrapper
    if adapter_tool and os.path.isdir(adapter_tool):
        (peft_model.load_adapter (
            adapter_tool,
            adapter_name="toolchain",
    ...
    # Default to extract adapter if present
    try:
        if "extract" in getattr(peft_model, "peft_config", {}):
            peft_model.set_adapter("extract")
    except Exception:
        pass

    return tok, peft_model

def ensure_adapter(model, adapter_name: Optional[str]):
    if adapter_name:
        try:
...
def generate_json_with_adapter(tok, model, prompt: str, max_new_tokens: int, adapter_name: Optional[str]):
    ensure_adapter(model, adapter_name)
    return generate_json(tok, model, prompt, max_new_tokens)

# ----------------------------
# UI
# ----------------------------
st.title("⚡ Mistral JSON Mapper – GPU 4-bit (Invoice / SOW) + Toolchain (Dual Adapters)")

with st.sidebar:
    st.header("Model Settings")
    base_model = st.text_input("Base model", value=DEFAULT_BASE_MODEL)
    adapter_extract = st.text_input("LoRA (Extraction) dir", value="")
    adapter_tool    = st.text_input("LoRA (Toolchain) dir", value="")
...
with st.spinner("Loading model/tokenizer on GPU (4-bit)…"):
    tok, model = load_models_gpu_dual_metasafe(
        base_model,
        adapter_extract if adapter_extract.strip() else None,
        adapter_tool if adapter_tool.strip() else None
    )
globals()["tok"] = tok
globals()["model"] = model
...
if run and uploaded_files:
    for up in uploaded_files:
        ...
        # 🔁 Use the "extract" adapter for mapping
        with st.spinner("Generating (extraction)…"):
            js, raw, sec = generate_json_with_adapter(tok, model, prompt, max_new_tokens=max_new, adapter_name="extract")

        c1, c2 = st.columns(2)
        with c1:
            st.caption(f"Took {sec:.1f}s on GPU (extract adapter)")
            if js is None:
                ...
                # ---- Run toolchain (Python functions by default)
                ...
                # If you want LLM-driven tool steps, uncomment & craft prompts, e.g.:
                # tool_prompt = "Use the toolchain policy to route approvals for: " + json.dumps(js)
                # _llm_out, _raw, _ = generate_json_with_adapter(tok, model, tool_prompt, 256, adapter_name="toolchain")
