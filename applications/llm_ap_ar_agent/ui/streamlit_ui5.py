...
with upload_tab:
    ...
    if uploaded_file is not None:
        ...
        query = st.text_input("Ask a question about the uploaded invoice:", f"Run {option} on this invoice")

with mock_tab:
    mock_file_map = {
        "Accounts Payable": "ap_mock_invoices.json",
        "Accounts Receivable": "ar_mock_invoices.json",
        "Invoice Verification": "ap_mock_invoices.json",
        "PO Matching": "ap_mock_invoices.json",
        "Approval Process": "ap_mock_invoices.json",
        "Payment Processing": "ap_mock_invoices.json"
    }
    mock_file = mock_file_map.get(option, "ap_mock_invoices.json")
    ...
    query = st.text_input("Ask a question or run agent task:", f"Run {option} for this invoice")
...
with image_tab:
    ...
    if uploaded_image is not None:
        ...
        query = st.text_input("Ask a question about the image-based invoice:", f"Run {option} on this image")
...
if st.button("Run Agent") and query:
    if option == "Invoice Verification":
        response = run_invoice_verification(document_data)
    elif option == "PO Matching":
        response = run_po_matching(document_data)
    elif option == "Approval Process":
        response = run_approval_process(document_data)
    elif option == "Payment Processing":
        response = run_payment_processing(document_data)
    else:
