...
if st.button("Run Agent"):
    if not query:
        ...
        if uploaded_invoice:
            ...
            mapped_data = map_extracted_text_to_invoice_data_with_confidence_score(invoice_text)
            st.subheader("Mapped  + Invoice data")
            st.json(mapped_data.get("mapped_invoice_details", mapped_data))

        # Process purchase order
        if uploaded_po:
            po_text, po_meta, po_ocr = retrieve_pdf_with_image_ocr(uploaded_po)
            st.success("Extracted purchase order text and image OCR.")
            st.text_area("🖼️ Extracted Text from Image", po_text, height=200)
            mapped_data = map_extracted_text_to_po_data_with_confidence_score(po_text)
            st.subheader("Mapped  + PO data")
            st.json(mapped_data.get("mapped_purchase_order_details", mapped_data))
        ...
        if not invoice_text and not po_text:
            ...
            mapped_data = map_extracted_text_to_invoice_data(combined_text)
            st.subheader("Mapped Invoice + PO data")
            st.json(mapped_data.get("combined_invoice_purchase_order_details", mapped_data))
            input_data = {
