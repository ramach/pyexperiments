...
def main():
    ...
    df = liquidity_analysis("/Users/krishnaramachandran/Downloads/synthetic", 'LC')
    ...
    collection = chroma_client.create_collection(name="my_chunked_collection", embedding_function=embedding_function)
    embeddings = OpenAIEmbeddings()
    for i in range(0, len(df), chunk_size):
        chunk = df[i:i + chunk_size]
        for index, row in chunk.iterrows():
            row_string = (f"act_symbol: {row['act_symbol']}, date: {row['date']},"
                          f" total_current_assets: {row['total_current_assets']},"
                          f" total_current_liabilities: {row['total_current_assets']},"
                          f" inventories: {row['inventories']},"
                          f" cash_and_equivalents: {row['cash_and_equivalents']},"
                          f" current_ratio: {row['current_ratio']},"
                          f" quick_ratio: {row['quick_ratio']},"
                          f" cash_ratio: {row['cash_ratio']},"
                          f" inventory_to_current_assets: {row['inventory_to_current_assets']}")
            collection.add(row_string)
    competitor_analysis = """
